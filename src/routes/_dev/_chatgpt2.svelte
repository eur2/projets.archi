<script>
	...
	// Define a function to apply filters and conditions
	function applyFilters(posts, searchTerm, selectTypeMulti, checkLaureat, filters) {
		return posts.filter((post) => {
			// Filter by search term
			const searchTermMatches = [
				post.acf.types.some((value) => value.toLowerCase().includes(searchTerm)),
				post.acf.projet.name.toLowerCase().includes(searchTerm),
				post.acf.localisation.toLowerCase().includes(searchTerm),
				post.acf.architecte.toLowerCase().includes(searchTerm),
				post.acf.architecte_associé.toLowerCase().includes(searchTerm),
				post.acf.amenageur.toLowerCase().includes(searchTerm),
				post.acf.plus.toLowerCase().includes(searchTerm),
				post.acf.maitre.toLowerCase().includes(searchTerm),
				post.acf.paysagiste.toLowerCase().includes(searchTerm),
				post.acf.bet_structure.toLowerCase().includes(searchTerm),
				post.acf.bet_environnement.toLowerCase().includes(searchTerm),
				post.acf.bet_thermique.toLowerCase().includes(searchTerm),
				post.acf.bet_fluides.toLowerCase().includes(searchTerm),
				post.acf.bet_general.toLowerCase().includes(searchTerm),
				post.acf.acousticien.toLowerCase().includes(searchTerm),
				post.acf.economiste.toLowerCase().includes(searchTerm),
				post.acf.eclairagiste.toLowerCase().includes(searchTerm)
			].some(Boolean);

			// Apply selectTypeMulti filter
			const selectTypeMultiMatches =
				selectTypeMulti.length === 0 ||
			...
			const additionalFiltersMatch = filters.every((filter) => filter(post));

			// Combine all filter conditions
			return searchTermMatches && selectTypeMultiMatches && additionalFiltersMatch;
		});
	}

	// Define additional filters
	const filters = [
		(post) => selectAnnee === 'all' || post.acf.annee == selectAnnee,
		(post) =>
			selectLocalisation === 'all' || post.acf.localisation.toLowerCase() === selectLocalisation,
		(post) => selectStructure === 'all' || post.acf.structure.toLowerCase() === selectStructure,
		(post) => selectSurface === 'all' || post.acf.surface.toLowerCase() === selectSurface,
		(post) => selectBudget === 'all' || post.acf.budget.toLowerCase() === selectBudget,
		(post) =>
		...
		(post) => selectMaitre === 'all' || post.acf.maitre.toLowerCase() === selectMaitre,
		(post) => selectAmenageur === 'all' || post.acf.amenageur.toLowerCase() === selectAmenageur,
		(post) => selectPaysagiste === 'all' || post.acf.paysagiste.toLowerCase() === selectPaysagiste,
		(post) =>
			selectBureauEtudes === 'all' ||
		...
		(post) => checkLaureat === false || post.acf.laureat === checkLaureat
	].filter(Boolean); // Remove the filters where 'all' is selected
...
<aside
	class="fixed b0 t0 r0 z4 sm bg-green p025 overflow-y"
	style="width:{!visible ? '170px' : '24px'};"
>
...
<footer class="center eeeee bg-fbfbfb mb p sm {!visible ? 'mr-open' : 'mr-close'}">
	...
	<!-- <div class="non">
		{#each filteredPosts as post}
			<a data-sveltekit-preload-data="hover" href="/p/{post.id}">{post.id}</a>
		{/each}
	</div> -->
</footer>
