<script>
  ...
  import { splash } from "$lib/store.svelte.js";
  ...
  let selectTypeMulti = $state([]);
  let checkLaureat = $state(false);
  let selectLocalisation = $state("all");
  let selectAnnee = $state("all");
  let selectStructure = $state("all");
  let selectSurface = $state("all");
  let selectBudget = $state("all");
  let selectMaitre = $state("all");
  let selectArchitecte = $state("all");
  let selectAmenageur = $state("all");
  let selectPaysagiste = $state("all");
  let selectBureauEtudes = $state("all");
    ...
    selectTypeMulti.length === 0
      ? posts
          .filter(
            (post) =>
              post.acf.types.some(
                (value) => value.toLowerCase().indexOf(searchTerm) !== -1,
              ) ||
              post.acf.projet.name.toLowerCase().indexOf(searchTerm) !== -1 ||
              (post.acf.localisation &&
                post.acf.localisation.toLowerCase().indexOf(searchTerm) !==
                  -1) ||
              (post.acf.localisation1 &&
                post.acf.localisation1.toLowerCase().indexOf(searchTerm) !==
                  -1) ||
              post.acf.architecte.toLowerCase().indexOf(searchTerm) !== -1 ||
              post.acf.architecte_associé.toLowerCase().indexOf(searchTerm) !==
                -1 ||
              ...
              post.acf.bet_environnement.toLowerCase().indexOf(searchTerm) !==
                -1 ||
          ...
          .filter((post) =>
            ...
            selectPaysagiste === "all"
              ...
              : (selectAnnee === "all" || post.acf.annee == selectAnnee) &&
                (selectLocalisation === "all" ||
                  (post.acf.localisation &&
                    post.acf.localisation.toLowerCase() ===
                      selectLocalisation) ||
                  (post.acf.localisation1 &&
                    post.acf.localisation1.toLowerCase() ===
                      selectLocalisation)) &&
                (selectStructure === "all" ||
                  post.acf.structure.toLowerCase() === selectStructure) &&
                (selectSurface === "all" ||
                  post.acf.surface.toLowerCase() === selectSurface) &&
                (selectBudget === "all" ||
                  post.acf.budget.toLowerCase() === selectBudget) &&
                (selectArchitecte === "all" ||
                  post.acf.architecte.toLowerCase() === selectArchitecte ||
                  post.acf.architecte_associé.toLowerCase() ===
                    selectArchitecte) &&
                (selectMaitre === "all" ||
                  post.acf.maitre.toLowerCase() === selectMaitre) &&
                (selectAmenageur === "all" ||
                  post.acf.amenageur.toLowerCase() === selectAmenageur) &&
                (selectPaysagiste === "all" ||
                ...
                (selectBureauEtudes === "all" ||
                  ...
                  post.acf.bet_environnement.toLowerCase() ===
                    selectBureauEtudes ||
      ...
      : posts
          .filter(
            (post) =>
              (post.acf.types &&
                post.acf.types.some((value) =>
                  selectTypeMulti.includes(value),
                )) ||
              (post.acf.type_construction &&
                post.acf.type_construction.some((value) =>
                  selectTypeMulti.includes(value),
                )),
          )
          .filter(
...
{#if splash.isOpen}
  <Splash {posts} onclose={() => (splash.isOpen = false)}>
    {@html header.content.rendered}
  </Splash>
{/if}
<div class="z5 t0 l0 p05 logo absolute">
...
<nav
  class="b0 l0 r0 z4 jc-center fixed flex {!visible ? 'mr-open' : 'mr-close'}"
>
  <a
    href="/carte"
    class="p05 m025 border-radius bg-black"
...
<aside
  class="b0 t0 r0 z4 sm bg-green p025 overflow-y fixed {!visible
    ? 'open'
    : 'close'}"
>
  <!-- <button onclick={handleToggle} class="absolute t0 b0 r0 w100 z-1" style="padding: 0;"></button> -->
  <div
    ...
    class="pointer t0 b0 r0 w100 z-1 absolute"
  ></div>
  <div class={!visible ? "open-content" : "close-content"}>
    ...
    <Select
      ...
      label="Structure"
      values={structures}
      bind:value={selectStructure}
    />
    <Select
      id="surface"
      label="Surface"
      values={surfaces}
      bind:value={selectSurface}
    />
    <Select
      id="budget"
      label="Budget"
      values={budgets}
      bind:value={selectBudget}
    />
    ...
    <Select
      ...
      values={maitres}
      bind:value={selectMaitre}
    />
    <Select
      id="amenageur"
      label="Aménageur"
      values={amenageurs}
      bind:value={selectAmenageur}
    />
    <div class="p025 column flex">
      <br />
      <label for="laureat"
        ><input
          type="checkbox"
...
<main class="bg-grey {!visible ? 'mr-open' : 'mr-close'}">
  <div class="header flex bg-white" style="padding-bottom: 0;">
  ...
  <div>
    {#if filteredPosts.length}
      {#each filteredPosts as post}
        <Post {post} />
      {/each}
    {:else}<div class="center p bg-white">Aucun résultat :(</div>{/if}
  </div>
</main>
<footer
  class="center eeeee bg-fbfbfb mb p sm {!visible ? 'mr-open' : 'mr-close'}"
>
