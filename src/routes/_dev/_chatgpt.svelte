<script>
	...
	function applyFilters(posts, filters) {
		return posts.filter((post) => {
			...
			const searchTermMatches = Object.values(filters.search).some((term) =>
				Object.values(post.acf).some((value) => {
			...
			const selectTypeMultiMatches =
				filters.selectTypeMulti.length === 0 ||
				(post.acf.types &&
					post.acf.types.some((value) => filters.selectTypeMulti.includes(value))) ||
				(post.acf.type_construction &&
					post.acf.type_construction.some((value) => filters.selectTypeMulti.includes(value)));

			// Apply additional filters dynamically
			const additionalFiltersMatch = Object.keys(filters.additionalFilters).every((filterName) => {
				const filterValue = filters.additionalFilters[filterName];
				if (filterValue === 'all') return true;
				const postValue = post.acf[filterName];
				if (typeof postValue === 'string') {
					return postValue.toLowerCase() === filterValue.toLowerCase();
				}
				return false;
			});
	...
	// Define the initial filter settings
	const initialFilters = {
		search: {
			types: '' // Add your search terms here
		},
		selectTypeMulti: [], // Add your selected types here
		additionalFilters: {
			annee: 'all',
			localisation: 'all',
			structure: 'all',
	...
	$: filteredPosts = applyFilters(posts, initialFilters);
</script>
